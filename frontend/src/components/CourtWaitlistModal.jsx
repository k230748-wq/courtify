...
export default function CourtWaitlistModal({ onClose }) {
  const [form, setForm] = useState({
    name: '', email: '', phone: '', court_name: '',
    city: '', number_of_courts: '', sports_offered: '',
  });
  ...
  const submit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/users/court-waitlist/`, {
        method: 'POST',
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(0,0,0,0.8)' }} onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h2 className="text-2xl font-bold text-white">Join as Court Partner</h2>
            <p className="text-white/50 text-sm">List your court on Courtify</p>
          </div>
        </div>
        ...
        {result ? (
          ...
          <form onSubmit={submit} className="space-y-4">
            ...
            <div>
              <label className="block text-white/70 text-sm mb-1">Court Name *</label>
              <input name="court_name" value={form.court_name} onChange={handle} required placeholder="e.g. Green Court Karachi" className={inputClass} />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
              ...
              <div>
                <label className="block text-white/70 text-sm mb-1">No. of Courts</label>
                <input type="number" name="number_of_courts" value={form.number_of_courts} onChange={handle} min="1" placeholder="1" className={inputClass} />
              </div>
            </div>
            <div>
              <label className="block text-white/70 text-sm mb-1">Sports Offered *</label>
              <input name="sports_offered" value={form.sports_offered} onChange={handle} required placeholder="e.g. Cricket, Football, Badminton" className={inputClass} />
            </div>
